...
class MaxxVit(nn.Module):
    ...
    def initialize_weights(self):
        maxvit_model_dict = self.state_dict()
        load_model_dict = torch.load('/18515601223/AVSegFormer/model/backbone/pytorch_model.bin')
        state_dict = {k: v for k, v in load_model_dict.items()if k in maxvit_model_dict.keys()}
        maxvit_model_dict.update(state_dict)
        self.load_state_dict(maxvit_model_dict)
        print(f'==> Load maxvit-base parameters pretrained on ImageNet')
    ...
    def forward(self, x):
        features = self.forward_intermediates(x, norm=True, stop_early=False, intermediates_only=True)
        return features
...
def _create_maxxvit(variant, cfg_variant=None, pretrained=False, **kwargs):
    ...
    model = MaxxVit(model_cfg)
    
    maxvit_model_dict = model.state_dict()
    load_model_dict = torch.load('/18515601223/AVSegFormer/model/backbone/pytorch_model.bin')
    load_model_dict = checkpoint_filter_fn(load_model_dict, model)
    state_dict = {k: v for k, v in load_model_dict.items()if k in maxvit_model_dict.keys()}
    maxvit_model_dict.update(state_dict)
    model.load_state_dict(maxvit_model_dict)
    print(f'==> Load maxvit-base parameters pretrained on ImageNet')
...
def _cfg(url='', **kwargs):
    return {
        'url': url, 'num_classes': 1000, 'input_size': (3, 512, 512), 'pool_size': (16, 16),
