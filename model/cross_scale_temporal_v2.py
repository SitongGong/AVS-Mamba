...
class VMamba_Block(nn.Module):
    ...
    def forward_core(self, x1: torch.Tensor, x2: torch.Tensor, x3: torch.Tensor):
        ...
        x_hwwh = torch.cat([x_hwwh1, x_hwwh2, x_hwwh3], dim=-1)          # bs, 2, dim, l1 + l2 + l3
...
class TemporalMambaBlock(nn.Module):
    ...
    def forward_core(self, input_x1: torch.Tensor, input_x2: torch.Tensor, input_x3: torch.Tensor):
        ...
        xs3_ = torch.cat([x1, x2, x3], dim=-1)            # bs, dim, 3 * hwt
        xs3 = torch.stack([xs3_, torch.flip(xs3_, dims=[-1])], dim=1)        # bs, 2, dim, 3 * hwt
        
        x1 = input_x1.permute(0, 1, 4, 3, 2).reshape(B, C, -1)       # bs, dim, wht
        x2 = input_x2.permute(0, 1, 4, 3, 2).reshape(B, C, -1)       # bs, dim, wht
        x3 = input_x3.permute(0, 1, 4, 3, 2).reshape(B, C, -1)       # bs, dim, wht
        xs4_ = torch.cat([x1, x2, x3], dim=-1)
        ...
        xs = torch.cat([xs1, xs2, xs3, xs4], dim=1)            # bs, 8, dim, l
        ...
        y3 = out_y[:, 4, ...] + torch.flip(out_y[:, 5, ...], dims=[-1]).view(B, -1, L)         # bs, dim, 3 * hwt
        ...
        y1 = out_y1_ + out_y1__ + out_y1___ + out_y1____
        y2 = out_y2_ + out_y2__ + out_y2___ + out_y2____
        y3 = out_y3_ + out_y3__ + out_y3___ + out_y3____
...
class Layer(nn.Module):
    def __init__(self,
        ...
        self.use_spatial_scale = use_spatial_scale
        ...
        if use_temporal_scale:
            ...
            self.temporal_mamba = TemporalMambaBlock(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state, use_conv_3d=use_conv_3d)
