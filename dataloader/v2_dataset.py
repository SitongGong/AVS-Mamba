...
def load_image_in_PIL_to_Tensor(path, split='train', mode='RGB', transform=None, cfg=None):
    ...
    if cfg.crop_img_and_mask:
        if split == 'train':
            img_PIL = crop_resize_img(cfg.crop_size, img_PIL, img_is_mask=False)     # traing set: center crop
        else:
            img_PIL = resize_img(cfg.crop_size, img_PIL, img_is_mask=False)    # validation/testing set: resize
    if transform:
...
class V2Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_base_path = os.path.join(
            self.cfg.dir_base, set, set, video_name, 'frames')
        audio_path = os.path.join(
            self.cfg.dir_base, set, set, video_name, 'audio.wav')
        binary_mask_base_path = os.path.join(self.cfg.dir_base, set, set, video_name, 'labels_semantic')
        color_mask_base_path = os.path.join(
            self.cfg.dir_base, set, set, video_name, 'labels_rgb')
        ...
        if self.split == "train":     
            for img_id in range(imgs_num):
                ...
                img_PIL = Image.open(img_path).convert('RGB')       # PIL
                mask_path = os.path.join(color_mask_base_path, "%d.png" % (img_id))
                
                if os.path.exists(mask_path):        # the first frame has the mask
                    mask = Image.open(mask_path).convert('RGB')         # PIL
                    mask = transforms.ToTensor()(mask)
                    img, mask = self.train_transforms(img_PIL, mask)      # data augmentation
                    mask = transforms.ToPILImage()(mask)
                    color_mask_PIL = crop_resize_img(self.cfg.crop_size, mask, img_is_mask=True)       # resize
                    color_label = color_mask_to_label(color_mask_PIL, self.v2_pallete)    # numpy
                    color_label = torch.from_numpy(color_label)       # Tensor [H, W]
                    color_label = color_label.unsqueeze(0)
                    labels.append(color_label)
                # no mask, data augmentation for the images
                else:               
                    img, _  = self.train_transforms(img_PIL, None)
                    color_label = torch.zeros_like(labels[-1])     # label all zeros
                    labels.append(color_label)
            
                img = crop_resize_img(self.cfg.crop_size, img, img_is_mask=False)     # PIL: traing set: center crop
                img = self.img_transform(img)       # totensor normalization
                imgs.append(img)            # tensor

            for _ in range(imgs_pad_zero_num):    
                img = torch.zeros_like(img)
                imgs.append(img)
                color_label = torch.zeros_like(labels[-1])
                labels.append(color_label)

        # all the images in the validation/testing set have the masks
        if self.split != 'train':
            if set == 'v2':
                mask_num = 10
            else:
            ...
            for mask_id in range(mask_num):
                img_path = os.path.join(img_base_path, "%d.jpg" % (mask_id))
                img = load_image_in_PIL_to_Tensor(img_path, split=self.split, transform=self.img_transform, cfg=self.cfg)
                imgs.append(img)
                mask_path = os.path.join(color_mask_base_path, "%d.png" % (mask_id))
                color_label = load_color_mask_in_PIL_to_Tensor(mask_path, v_pallete=self.v2_pallete, split=self.split, cfg=self.cfg)
                labels.append(color_label)
        ...
        return imgs_tensor, audio_path, labels_tensor, vid_temporal_mask_flag, gt_temporal_mask_flag, video_name
