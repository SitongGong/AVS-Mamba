...
def _batch_miou_fscore(output, target, nclass, T, beta2=0.3):
    ...
    for i in range(target.shape[0]):
        ...
        area_inter = torch.histc(
            intersection[i], bins=nbins, min=mini, max=maxi).cpu()  # TP
        area_pred = torch.histc(
            predict[i], bins=nbins, min=mini, max=maxi).cpu()  # TP + FP
        area_lab = torch.histc(
            target[i], bins=nbins, min=mini, max=maxi).cpu()  # TP + FN
        # end1 = time.time()
...
def calc_binary_miou(pred, target, eps=1e-7, size_average=True):
    ...
    N = pred.size(0)
    num_pixels = pred.size(-1) * pred.size(-2)      # H * W
